    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.model.training:
            ...
            if self.state.global_step > 0 and self.state.global_step % self.args.logging_steps == 0:
                ...
                total_steps = self.args.num_train_epochs * len(self.get_train_dataloader())
                current_step = self.state.global_step + self.state.epoch * len(self.get_train_dataloader())
                estimated_total_time = elapsed_time / current_step * total_steps
                time_remaining = estimated_total_time - elapsed_time
                progress_percentage = 100 * current_step / total_steps
                
                log_info = ', '.join(map(str, [self.mrr_meter, self.acc1_meter, self.acc3_meter]))
                time_info = f"Elapsed: {elapsed_time:.2f}s, Remaining: {time_remaining:.2f}s, Progress: {progress_percentage:.2f}%"
                logger.info(f'step: {self.state.global_step}, {log_info}, {time_info}')
